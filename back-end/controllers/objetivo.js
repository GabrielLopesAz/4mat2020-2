...
controller.novo = async (req, res) => {
    try {
        ...
        await Objetivos.create(req.body)
        //http 201 : Criado
        res.status(201).end()
    }
    catch (erro) {
...
controller.listar = async (req, res) => {
    try {
        ...
        let dados = await Objetivos.find().populate('categoria', 'nome')
...
controller.obterUm = async(req,res) => {
    ...
    let obj = await Objetivos.findById(id)
...
controller.atualizar = async (req, res) => {
    try {
        ...
        let obj = await Objetivos.findByIdAndUpdate(id, req.body)//Busca o objeto pelo id e, encontrando-o substitui o conteudo por req.body
...
controller.excluir= async (req,res) =>{
   try{
    ...
    const id=req.body._id
    let obj = await Objetivos.findByIdAndDelete(id)
